...
```
我已答完 D 澄清。请按 /prompt/D-development/D01-D03-AI输出-数据规范.md 多文件结构输出，
落盘到 docs/D01-data/<feature-id>/。
所有命名/通用字段/主键策略必须遵守 B01 03-database。
不重定义 B02 04-user-data-model 中的认证表。
...
> **阶段**：D · 数据建模师
> **关联 R-ID**：R-XXX, R-XXX
> **上游**：本 feature C01 R 基线、B01 03-database、B02 04-user-data-model、本 feature C04 原型 + C05 PRD、D-<feature-id>-questions-resolved
> **冻结状态**：未冻结
...
```markdown
<!-- TARGET-PATH: docs/D01-data/<feature-id>/03-state-machines.md -->
...
```markdown
<!-- TARGET-PATH: docs/D01-data/<feature-id>/04-business-rules.md -->
...
```markdown
<!-- TARGET-PATH: docs/D01-data/<feature-id>/05-indexes.md -->
...
```markdown
<!-- TARGET-PATH: docs/D01-data/<feature-id>/06-volume-growth.md -->
