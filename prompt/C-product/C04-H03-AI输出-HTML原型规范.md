...
```
请你扮演「前端原型工程师」，遵循 /prompt/A-framework/A00-01、/prompt/A-framework/A00-03、/prompt/A-framework/A00-04，硬性遵守 docs/B02-permissions/、docs/B03-ux/、docs/B04-design/design-system/、docs/B04-design/prototype-style/、docs/C02-ia/<feature-id>/、docs/C03-pages/<feature-id>/。
...
2. **字体自托管**（除非 X 显式允许 CDN；字体走 vendor/proto-style/ 中的约定）。
...
// 仅本 feature 的页面级逻辑
window.feature = {
  goto(pageId, state = 'default') { location.href = `./${state === 'default' ? 'pages' : 'states'}/${pageId}${state === 'default' ? '' : '.' + state}.html`; },
