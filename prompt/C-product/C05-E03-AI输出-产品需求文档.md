...
> **谁产出**：AI（产品经理）
> **何时产出**：用户回完 E 澄清清单后。
> **落盘**：`docs/C05-prd/<feature-id>/` 下多个文件（见 `S00-04` 文档目录规划）
...
```
我已答完 E 澄清。请按 /prompt/C05-E03-AI输出-产品需求文档.md 的多文件结构输出，
...
> 仅给"表名 + 一句职责 + 关键关系"。字段详情请回查 docs/C06-data/<feature-id>/。
...
> 仅给"endpoint + 方法 + 一句职责"。请求 / 响应字段请回查 docs/C07-api/<feature-id>/。

| 模块 | endpoint | 方法 | 一句职责 | 角色 | 详情链接 |
|-----|---------|------|---------|------|---------|
| M-001 | /api/... | GET | | | docs/C07-api/<f>/02-endpoints/get-xxx.md |
```
...
> 数据来源：docs/F02-permissions/03-authz-mechanism.md。本表为简化矩阵，细则回查上游。
...
## 认证流程速记
（一段话 + 一张 mermaid 时序图，引自 docs/F02-permissions/02-auth-flow.md）
```
...
> 数据来源：docs/F03-ux/、docs/F04-design-system/。本文件只摘要，不复述全文。
...
> 数据来源：docs/F01-architecture/。本文件给"接手者最快上手版"。
...
## 2. 仓库目录速记
（一段树形 + 一行注释，详情回查 docs/F01-architecture/02-project-structure.md）
