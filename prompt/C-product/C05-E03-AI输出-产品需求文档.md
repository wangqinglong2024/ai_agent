...
1. **只读**：E 用户输入 + E-questions-round*-resolved + 本 feature 的 R / I / N / H 全部冻结产物 + B01 / B02 / B03 / B04 全部冻结产物 + `A00-01` + `A00-03` + `A00-04` + 本模板。
   > **严禁读**：D01 / D02 / D03 未产出，不得在 PRD 里描述表结构/接口文档/校验结果；
   > 08-data-model-summary / 09-api-summary / 10-roles-permissions 等子文件在本阶段只出「占位 + 待 D/L 产出后回填」说明。
...
> 所属模块：M-XXX · 路由：/xxx · 上游引用：C02-ia/02-pages.md §X、C03-pages/<f>/<page-id>.md、C04-prototype/pages/<scope>-<page>.html
...
## 文件 9：`08-data-model-summary.md`（占位件，待 D01 产出后回填）

```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/08-data-model-summary.md -->

# 数据模型摘要

> 本阶段（C05-E）仅产出占位。完整内容在 D01-D 冻结后由同一名产品经理角色回填（仅拼装 D01 产物的「表名 + 一句职责 + 关键关系」，不重复 D01 全文）。
> 未回填之前本文件 99 节必须保留提醒。

## TBD
- [ ] 待 D01 冻结后以 ER 图 + 表清单表填充。
- [ ] 字段详情一律以链接形式指向 docs/D01-data/<feature-id>/01-tables.md，不复制。
```

---

## 文件 10：`09-api-summary.md`（占位件，待 D02 产出后回填）

```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/09-api-summary.md -->

# 接口摘要

> 本阶段仅产出占位。完整内容在 D02-L 冻结后回填（仅「endpoint + 方法 + 一句职责」）。

## TBD
- [ ] 待 D02 冻结后表填充；详情链到 docs/D02-api/<feature-id>/02-endpoints/<endpoint-id>.md。
```
...
```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/10-roles-permissions.md -->
...
```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/11-design-summary.md -->
...
> 本文件**在 PRD 完成后、开发启动前**才补写。PRD（C05）作为产品设计阶段产物，不依赖架构详细。架构规范请直接查 docs/B01-architecture/，本文件只提供接手者速记。
```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/12-tech-stack-summary.md -->

# 技术栈摘要 + 接手第一周环境步骤

> 数据来源：docs/B01-architecture/。本文件给"接手者最快上手版"。
...
## 2. 仓库目录速记
（一段树形 + 一行注释，详情回查 docs/B01-architecture/02-project-structure.md）
...
```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/13-known-issues.md -->
...
```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/14-roadmap.md -->
...
```markdown
<!-- TARGET-PATH: docs/C05-prd/<feature-id>/15-changelog.md -->
