# 29 · I03 AI 输出：信息架构（功能·流程·状态机·页面·导航）模板
...
1. **只读**：I 用户输入 + 本 feature R 基线 + B02 全部 + B03 / B04 索引 + I-questions-round*-resolved + `A00-01` + `A00-03` + `A00-04` + 本模板。
   > **严禁读**：docs/B01-architecture/、docs/D01-data/、docs/D02-api/、docs/D03-validation/（C 与 D 隔离；I 阶段不依赖 B01 架构与任何 D 阶段产物）。
...
**严禁输出 URL / 路由 / API / 表字段**——这些在 D02 / D01 阶段产出。
```
...
> - D01 D：表设计参考本目录 03-state-machines.md（状态字段枚举）
> - D02 L：路由设计基于 04-pages.md 的 page-id 反向映射 URL；接口设计需覆盖 03-state-machines.md 的所有状态转移
...
> 本文件是 D01 数据模型阶段的 **状态字段枚举来源**、D02 接口设计阶段的 **状态转移接口覆盖检查表**。
...
> 本文件只描述"有哪些页面、谁能看、为什么需要"。**URL 路径在 D02 阶段映射**。
...
> 链接一律用 page-id 表达；URL 在 D02 阶段映射。
...
- [ ] **未出现 URL / 路由 / API / 表字段**？（违规 → 删除并放 §99 标注"应在 D02 / D01 阶段定义"）
- [ ] 系统兜底页齐？
