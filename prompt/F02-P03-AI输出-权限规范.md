...
```
我已答完 P 澄清。请按 /prompt/F02-P03-AI输出-权限规范.md 多文件结构输出到 docs/F02-permissions/。
认证方案必须与 A 输出 01-tech-stack 中的鉴权基础设施一致。
角色枚举必须与 F02 用户输入 §1（系统有哪些角色）一致；不允许擅自新增或删减。
...
```markdown
<!-- TARGET-PATH: docs/F02-permissions/00-index.md -->

# 权限与认证规范 · 索引

> **阶段**：P · 安全/权限工程师
> **上游依赖**：F01 架构规范, F02 用户输入(roles-input.md), P-questions-resolved
> **冻结状态**：未冻结
...
```markdown
<!-- TARGET-PATH: docs/F02-permissions/01-roles.md -->

# 角色定义

## 角色枚举

| 角色 ID | 显示名 | 来源（F02 用户输入条目）| 优先级 |
|--------|-------|---------------|--------|
| ROLE-USER | 普通用户 | ... | 1 |
| ROLE-EDITOR | 内容编辑 | ... | 50 |
| ROLE-ADMIN | 超管 | ... | 99 |
...
```markdown
<!-- TARGET-PATH: docs/F02-permissions/02-auth-flow.md -->
...
```markdown
<!-- TARGET-PATH: docs/F02-permissions/03-authz-mechanism.md -->
...
```markdown
<!-- TARGET-PATH: docs/F02-permissions/04-user-data-model.md -->
...
```markdown
<!-- TARGET-PATH: docs/F02-permissions/05-registration.md -->
...
```markdown
<!-- TARGET-PATH: docs/F02-permissions/99-open-questions.md -->
